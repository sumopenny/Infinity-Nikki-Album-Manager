...
export function useSelectionState(options: {
  ...
  const toggleAll = () => {
    ...
    selectedIds.value = allSelected.value ? new Set([...selectedIds.value].filter((id) => !ids.has(id))) : new Set([...selectedIds.value, ...ids])
  }
  const toggleAllOutfits = () => {
    const ids = new Set(visibleOutfits.value.map((outfit) => outfit.id))
    selectedOutfitIds.value = allOutfitsSelected.value ? new Set([...selectedOutfitIds.value].filter((id) => !ids.has(id))) : new Set([...selectedOutfitIds.value, ...ids])
  }
  const toggleOutfit = (id: string) => {
    const next = new Set(selectedOutfitIds.value); next.has(id) ? next.delete(id) : next.add(id); selectedOutfitIds.value = next
  }
  ...
  const toggleTrashPhoto = (id: string) => {
    const next = new Set(trashSelectedIds.value); next.has(id) ? next.delete(id) : next.add(id); trashSelectedIds.value = next
  }
  const toggleAllTrash = () => { trashSelectedIds.value = allTrashSelected.value ? new Set() : new Set(recentlyDeleted.value.map((photo) => photo.id)) }
  const toggleFavorite = (id: string) => {
    const next = new Set(favoriteIds.value); next.has(id) ? next.delete(id) : next.add(id); favoriteIds.value = next
    if (activeView.value === 'favorites' && !next.has(id)) selectedIds.value = new Set([...selectedIds.value].filter((selectedId) => selectedId !== id))
  }
  const favoriteSelectedPhotos = () => { favoriteIds.value = new Set([...favoriteIds.value, ...scopedSelectedPhotos.value.map((photo) => photo.id)]) }
  const unfavoriteSelectedPhotos = () => {
    ...
    if (activeView.value === 'favorites') selectedIds.value = new Set([...selectedIds.value].filter((id) => !ids.has(id)))
  }
  const clearAlbumSelection = () => { selectedIds.value = new Set() }
  const clearOutfitSelection = () => { selectedOutfitIds.value = new Set() }
  const clearTrashSelection = () => { trashSelectedIds.value = new Set() }
  const toggleDate = (dateKey: string) => {
    const group = dateGroups.value.find((item) => item.dateKey === dateKey); if (!group) return
    const next = new Set(selectedIds.value); const selected = group.photos.every((photo) => next.has(photo.id))
    for (const photo of group.photos) selected ? next.delete(photo.id) : next.add(photo.id)
    selectedIds.value = next
  }
  return { scopedSelectedPhotos, selectedCount, selectedOutfitCount, trashSelectedCount, allSelected, allOutfitsSelected, allTrashSelected, allSelectedFavorited, toggleAll, toggleAllOutfits, toggleOutfit, togglePhoto, toggleTrashPhoto, toggleAllTrash, toggleFavorite, favoriteSelectedPhotos, unfavoriteSelectedPhotos, clearAlbumSelection, clearOutfitSelection, clearTrashSelection, toggleDate }
}
