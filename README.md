...
- 照片缩略图和大图预览提供参数解析入口；右上角“工具”菜单中的“参数/搭配码解析”窗口集中提供 CameraParams 参数串和搭配码输入。解析完整照片时会读取已授权 X6Game 下的全部账号目录 UID，并依次尝试解密；解析失败后可手动填写 UID，成功 UID 会持久保存。解析在浏览器内通过 AssemblyScript/WASM 完成，不上传照片；参数窗口不显示照片背景或照片预览，展示拍摄时间、天气、焦距、光圈、晕影、画面调节、普通动作、灯光、滤镜、大喵动作及照片内保存的原始 CameraParams。拍摄时间和天气仅在完整照片包含对应 PhotoInfo 时显示，天气枚举会转换为晴天、雨天、彩虹或星海等名称。参数展示参考上游 nikki_albums 按字段格式化，比例类显示百分比、连续参数保留 1 位小数，滑杆位置按上游字段范围计算（无符号参数 0～1、有符号参数 -1～1、光圈 1～15、焦距原始值 0～1/完整照片 10～55mm），圆点两端保持在轨道内，底层原始值和重新编码精度不变。灯光、滤镜、大喵动作和普通动作的资源缩略图按 96×96 显示（源图通常为 128×128）；名称使用从 `nikki_albums` 热更新数据库或 NikkiGallery 动作目录导出的资源目录，未知资源保留原始 ID 并显示占位图。当前相册目录无需位于 X6Game 路径下。
...
- 通过右上角“工具”菜单中的“抽卡吉时”查看 2.10 版本娱乐性玄学时间表；实际出率仍以游戏概率为准。
...
- 搭配卡片右上角始终显示解析按钮；未填写搭配码时按钮保持禁用。删除按钮位于解析按钮左侧，并在悬停卡片或键盘聚焦时显示。
- 可从右上角“工具”菜单打开“参数/搭配码解析”窗口，直接输入搭配码，无需先创建搭配方案；解析结果复用同一个搭配码解析窗口。
...
- 删除过程中如果文件无法完整恢复，网站会弹窗列出相关方案，请手动检查当前相册的 `clothe` 文件夹。
...
点击右上角“工具”菜单中的“专项清理”打开清理窗口，使用清理功能需要授权 `X6Game` 文件夹。
...
- 照片在本地浏览器中读取。
- 照片尾段、账号密钥派生、AES 解密和 CameraParams 解析均在本地 WASM 中执行，照片和解析结果不会上传。
...
npm run build:cloudflare # 刷新普通动作目录后构建
...
Cloudflare Pages 的构建命令应设置为 `npm run build:cloudflare`，构建输出目录保持 `dist`。该命令每次构建先请求 NikkiGallery 版本接口；仅在版本变化或本地动作目录缺失时下载完整数据，并只更新普通动作字段。接口超时、返回错误或数据校验失败时会保留仓库内已有目录并继续构建。需要恢复为完全离线、可复现的静态构建时，将 Cloudflare 构建命令改回 `npm run build`。

可选环境变量：`NIKKIGALLERY_API_BASE` 用于覆盖接口根地址，`NIKKIGALLERY_API_TIMEOUT_MS` 用于调整请求超时毫秒数。手动严格检查可执行 `npm run resources:update:poses -- --strict`，此模式下同步失败会返回非零退出码。
