...
describe('redesigned album controls', () => {
  ...
  it('keeps top menus mutually exclusive and opens help from More', async () => {
    ...
    await wrapper.findAll('.header-dropdown button').find((button) => button.text() === messages.zh.topBar.feedback)?.trigger('click')
    const feedbackFrame = wrapper.get('.feedback-iframe')
    expect(feedbackFrame.attributes('src')).toBe('https://v.wjx.cn/vm/tUM7gga.aspx')
    expect(feedbackFrame.attributes('referrerpolicy')).toBe('strict-origin-when-cross-origin')
    expect(wrapper.get('.feedback-external-link').attributes('href')).toBe('https://v.wjx.cn/vm/tUM7gga.aspx')
